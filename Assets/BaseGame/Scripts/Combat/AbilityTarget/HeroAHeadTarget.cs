...
namespace Combat
{
    [CreateAssetMenu(menuName = "AbilityTarget/HeroAHeadTarget")]
    public class HeroAHeadTarget : AbilityTarget
    {
        ...
        private bool TryFindTargetInBattleState(Hero hero, out Holder[] holders, out int count)
        {
            ...
            if (count >= MaxTarget) return true;
            Holder[] opponentBattleSlot = GetOpponentBattleSlot(hero);
            for (int i = 0; i < opponentBattleSlot.Length; i++)
            {
                if (!opponentBattleSlot[i].HasHero) continue;
                if (opponentBattleSlot[i].OwnerHero.IsFainted) continue;
                holders[count] = opponentBattleSlot[i];
                count++;
                if (count >= MaxTarget) break;
            }
