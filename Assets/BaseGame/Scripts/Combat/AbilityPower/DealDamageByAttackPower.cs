...
namespace Combat
{
    public class DealDamageByAttackPower : AbilityPower
    {
        ...
        public override async UniTask Resolve(Hero hero, TickRate tickRate, CancellationToken cancellationToken)
        {
            ...
            for (int i = 0; i < LoopTime; i++)
            {
                ...
                for (int j = 0; j < count; j++)
                {
                    ...
                    holders[j].OwnerHero.WillTakeDamage(DamageScale * hero.AttackDamage);
        ...
        {
            await to.TakeDamage(DamageScale * from.AttackDamage, from.Transform.position, to.Transform.position, tickRate, cancellationToken);
        }
    }
}