...
namespace Combat
{
    public class GiveFaintTriggerMorePower : AbilityPower
    {
        ...
        public override AbilityPower UpdateData(Dictionary<string, string> data)
        {
            ...
            if (int.TryParse(data["Value0"], out int value1))
            {
                LoopTime = value1;
            }
        ...
        private UniTask OnProjectileMoveComplete(Hero from, Hero to, TickRate tickRate, CancellationToken cancellationToken)
        {
            to.SetFaintLoopTime(LoopTime);
